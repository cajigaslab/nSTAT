...
- **D0.3 — Orphans:**
  ...
  - 8 `.m` not in `helptoc.xml`: 7 user-facing pages needed TOC entries (`AnalysisExamples2`, `ConfidenceIntervalOverview`, `FitResultReference`, `FoundationModelKSValidation`, `HelloNstat`, `HybridFilterExample`, `WhenToUseWhich`) plus 1 intentionally private helper (`publish_all_helpfiles`). Remediation: 9 new TOC entries (including a new "Analysis Reference" entry) added to `helpfiles/helptoc.xml`.
