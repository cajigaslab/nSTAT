...
%            end
        ...
        function computeStatistics(nstObj,makePlots)
            ...
            if(~isempty(ISI))
                ...
                %B has the value of ?1 for regular time series as ? = 0, and 0 for Poissonian or random time series as ? = �. Finally, the value of B approaches 1 for extremely bursty time series as ? ? ? for finite �.
                n=length(spikeTimes);
    ...
    %             end
                ...
                if(~isempty(burstStart))
                    if(makePlots==1)
                        figure; % FIX: replaced 'close all' — closing all figures is a destructive side effect
                        nstObj.plot; hold on;
                        plot(tdiff, ISI,'ko');
...
%             binWidth=1/numBins;
            if(~isempty(ISIs))
                ...
                % histogram() includes the right edge in its last bin, so
                % subtract those from h.Values(end) to avoid double-counting.
                n_at_edge = sum(ISIs == bins(end));
                counts = [h.Values, 0];
                counts(end-1) = counts(end-1) - n_at_edge;
                counts(end) = n_at_edge;
            else
                counts = [];
            end
